...
def lambda_handler(event, context):
    ...
    print("Compare HTTP and HTTPS rules")
    print(http_listener['Rules'])
    print(https_listener_rules)
    ...
    while i < len(https_listener_rules):
        ...
        while n < len(actions):
            try:
                if actions[n]['Type'] == "forward" and check_target_update(actions[n]['TargetGroupArn'], arr_available_target_groups):
                    actions[n]['TargetGroupArn']=http_target_group_arn
                    actions[n]['ForwardConfig']['TargetGroups'][0]['TargetGroupArn'] = http_target_group_arn
                    modify_rule=1
        ...
        if modify_rule==1:
            ...
            print(https_listener_rules[i]['RuleArn'])
            print(actions)
            try:
